import { MutableRefObject, ReactElement, UIEvent, useEffect, useMemo, useState } from 'react'
import { faEnvelope, faSearch, faTh, faUser } from '@fortawesome/free-solid-svg-icons'
...
const Header = ({ scrollRef, offsetY = 100 }: Props): ReactElement => {
    ...
    const items = useMemo<ReactElement>(() => <>
        ...
        <span></span>
        <Item text='' href='#top' icon={faSearch} />
    </>, [])

    return <div className={`${header} ${offsetMore ? header_scroll : ''}`}>
        {items}
    </div>
}
