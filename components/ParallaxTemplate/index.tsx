...
const ParallaxTemplate = (): ReactElement => {
    ...
    const { parallax_block, parallax_block_first, logo, parallax_block_second } = style

    const scrollRef = useRef(null!)

    const relativeImg = (src: string): string => `images/ParallaxTemplate/${src}`
    return <div className={`index ${parallaxtemplate}`} ref={scrollRef}>
        <Header scrollRef={scrollRef} />
        <Parallax id='top' src={relativeImg('parallax1.jpg')}
        ...
        <About />
        <Info />
        <Parallax id='portfolio' src={relativeImg('parallax2.jpg')}
            className={`${parallax_block} ${parallax_block_second}`}>
            <div>PORTFOLIO</div>
        </Parallax>
        <Work />
        <Parallax id='contact' src={relativeImg('parallax3.jpg')}
            className={`${parallax_block} ${parallax_block_second}`}>
            <div>CONTACT</div>
        </Parallax>
        <Where />
